...
jQuery(document).ready(function($) {
	...
	var penguin = new Object({
		...
		tabs:{ init:function(){
				...
				setTimeout(removePenguinOver,2000);
				...
				function backDragElement(params){
					...
					$(params).find('.penguin-drag-check').click(function() {
                        if(!$(this).hasClass('show')){
							$(this).addClass('show');
							if($(this).parent().prevAll('.penguin-drag-check-position').length > 0){
								$(this).parent().prevAll('.penguin-drag-check-position').find('.penguin-drag-check').removeClass('show');
							}
							
							if($(this).parent().nextAll('.penguin-drag-check-position').length > 0){
								$(this).parent().nextAll('.penguin-drag-check-position').find('.penguin-drag-check').removeClass('show');
							}
						}else{
